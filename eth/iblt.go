...
func (b ibitset) Set(i int) {
	b[i>>6] |= 1 << (uint32(i) & 63)
}

func (b ibitset) Clear(i int) {
	b[i>>6] &= ^(1 << (uint32(i) & 63))
}

func (b ibitset) Test(i int) bool {
	return (b[i>>6] & (1 << (uint32(i) & 63))) != 0
}

func (b ibitset) ClearAll() {
	for i := range b {
		b[i] = 0
	}
}

func newiBitset(l int) ibitset {
